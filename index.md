...
- I trained a probe on Qwen2.5-1.5B's activations on an affirmative dataset on city/country facts and achieved near-perfect results by testing on other affirmative city/country facts. However, I also tested the same probe on negations (e.g. "The city of Tokyo is not in Italy") and found it systematically ranks true statements below false ones, achieving an accuracy far below chance in the middle layers (reproducing [Marks and Tegmark, 2023](https://arxiv.org/abs/2310.06824), and consistent with the mechanism [Bürger et al., 2024](https://arxiv.org/abs/2407.12831) identify). It seems the probe learned to output true for a correct city/country pair; therefore, by adding a negation, the probe produces exactly the opposite answer.
- I tested the same probe on compound statements (two statements joined by AND or OR) and found that the probe not only inverts negations, but also ignores the connective (AND or OR) entirely. It seems to report roughly the average of the factual association of each of the two halves. That blindness only shows up when AND and OR are pooled, and neither [Bürger et al. (2024)](https://arxiv.org/abs/2407.12831) nor [Bao et al. (2025)](https://arxiv.org/abs/2506.00823) report that split.
...
This reproduces [Marks and Tegmark (2023)](https://arxiv.org/abs/2310.06824) at smaller scale; the layer-by-layer view adds that the failure is concentrated in a band rather than uniform. Below-chance accuracy is not ignorance; a probe at 1% accuracy predicts the opposite label 99% of the time.
...
Additionally, the in-distribution accuracy saturates from layer 10 onward (layers 10–27), spanning 0.990 to 0.995. However, transfer AUROC to negated statements over the same layers has a much higher range from 0.0001 to 0.334. A layer-selection criterion based on accuracy would only have a 0.5% spread to work with, and within that range, the AUROC differs by roughly 60 times more. Even within those layers, there is no good choice as the best layer sits well below chance.
...
For the experiment, I calculate compound scores using the probe's `decision_function` output, z-scored against the distribution of *cities* scores at the same layer, so that layers are comparable and zero corresponds to the average atomic statement. In general terms, the sign of the score corresponds to whether the model thinks the statement is true (positive = true) and the magnitude of the score represents confidence. Each of two hypotheses makes distinct predictions on the compound scores.
...
Both [Bürger et al.](https://arxiv.org/abs/2407.12831) and [Bao et al. (2025)](https://arxiv.org/abs/2506.00823) evaluate compound statements within-connective, where conjunct-counting alone scores well (under AND the only true class (TT) has the most true conjuncts, under OR the only false class (FF) has the fewest). Bürger et al.'s Figure 5 (the result that adding negations improves compound separation) uses AUROC, a threshold-free metric that can't separate counting from truth-reading. Their Figure 6b uses accuracy which is a harder test since there is one threshold for both AND/OR. A probe that only counts true conjuncts would struggle with accuracy because AND and OR need the boundary in different places.
...
The mixed patterns flip sign with the connective, which is consistent with operator-reading. TT/FF have the same truth value under both connectives yet those scores move as much as the mixed patterns do. The connective shifts every cell by about the same amount (+1.05 to +1.31) regardless of truth value. This shift is a constant offset, not an interaction. Genuine composition understanding would mean the connective changes how the conjuncts combine, implying the shift would depend on conjunct pattern. However, the score decomposes into a conjunct term plus a connective term, added independently. The new probe still classifies all eight cells correctly, so nothing in this dataset separates it from genuine compositional understanding. (Nothing could separate it under this setup. With two conjuncts, AND and OR are both thresholds on the conjunct count, so an additive count plus offset model matches both truth tables. That also means a model can pass a shared-threshold test like Bürger et al.'s Figure 6b, since the offset pushes the OR scores past the same boundary. Separating the two accounts needs mixed connectives like "(A and B) or C", where truth isn't a function of the count, which is left to future work.)
...
Adding in negations to the training data did produce a different direction that handles negations, but this new direction still does not handle compounds. That result does not reproduce [Bürger et al.'s (2024)](https://arxiv.org/abs/2407.12831) Figure 5, where they showed a probe trained on the union of affirmative and negated statements separates compound statements better than one trained on affirmative alone.

However, our metrics may not be comparable. My evaluation pools across connectives and theirs is within-connective, which is an easier test. It is possible to have high within-connective AUROC scores and low pooled scores: my probe trained on compounds, which the per-cell breakdown above shows is blind to the connective, still reaches 0.84–0.96 within AND and 0.72–0.92 within OR while only achieving 0.50–0.82 pooled.

If the results still disagree after that, three other differences could explain it. 1) I use a single topic (*cities*) while they balance across 6 topics. Their analysis shows single-topic training leaves topic-specific features correlated with truth. 2) I also use a much smaller model (Qwen2.5-1.5B) than them (7B–27B). 3) Their probe is also built differently from mine. Their direction uses a mass-mean probe (taking the difference in means of activations of true statements vs false ones) while I use an L2-regularized logistic regression. Bürger et al. show these two methods have different strengths: logistic regression is slightly ahead on unseen topics of familiar form (94.6% vs 92.2%) while mass-mean is well ahead on real-world lies (90.5% vs 79%). Compound statements are a change in form, not just topic.

The claim I can support is limited: an L2-regularized logistic regression probe trained on the union did not transfer to a pooled compound test at 1.5B under single-topic training.

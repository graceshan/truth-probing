...
Both [Bürger et al.](https://arxiv.org/abs/2407.12831) and [Bao et al. (2025)](https://arxiv.org/abs/2506.00823) evaluate compound statements within-connective, where conjunct-counting alone scores well (under AND the only true class (TT) has the most true conjuncts, under OR the only false class (FF) has the fewest). Bürger et al.'s Figure 5 (the result that adding negations improves compound separation) uses AUROC, a threshold-free metric that can't separate counting from truth-reading. Their Figure 6b uses accuracy which is a harder test since there is one threshold for both AND/OR. A connective-blind probe can't pass that, because AND and OR need the boundary in different places.
...
A conjunct count plus a constant connective offset can pass a shared-threshold test like Bürger et al.'s Figure 6b, since the offset pushes the OR scores past the same boundary.

...
- A linear probe is a classifier trained on a language model's internal activations. If it can separate out when the language model is reading true statements versus false ones, the idea is that the idea of truth is encoded in the model and we can read it directly with the probe instead of trusting what the model says. Linear probes have been proposed as safety monitors for situations like catching a model that is about to behave deceptively.
- However, what a probe learns is limited to its training data so its notion of truth depends on the training data. For example, in affirmative datasets (simple statements without negation / compound logic, e.g. "The city of Tokyo is in Japan"), truth coincides with a correct city-country pairing. Therefore, a probe scoring 100% may have actually learned truth, or it may have learned a shortcut correlated with truth in the training data, such as "do the things in the sentence go together."
- I trained a probe on Qwen2.5-1.5B's activations on an affirmative dataset on city/country facts and achieved near-perfect results by testing on other affirmative city/country facts. However, I also test the same probe on negations (e.g. "The city of Tokyo is not in Italy") and find it systematically ranks true statements below false ones, achieving an accuracy far below chance in the middle layers (reproducing Marks and Tegmark, 2023). It seems the probe learned to output true for a correct city/country pair; therefore, by adding a negation, the probe produces exactly the opposite answer.
- I test the same probe on compound statements (two statements joined by AND or OR) and find that the probe not only inverts negations, but also ignores the connective (AND or OR) entirely. It seems to report roughly the average of the factual association of each of the two halves.
- However, standard metrics don't reveal these findings. At one layer, accuracy is 0.500, which looks like a probe with no signal, when in reality it sorts true statements below false ones almost perfectly. Another example is how a probe trained directly on compound statements scores 96%, which looks like it successfully handles compositional logic, when on closer inspection the model is getting there without representing what AND and OR mean.
